...
class AppStore extends Component {
  state = {activeTab: tabsList[0], searchInput: ''}

  getUpdatedActiveTab = id => {
    const updateActiveTab = tabsList.filter(eachTab => eachTab.tabId === id)
    this.setState({activeTab: updateActiveTab[0]})
  }
        ...
        <ul className="tab-cont">
          {tabsList.map(tab => (
            <TabItem
              tabDetails={tab}
              key={tab.tabId}
              getUpdatedActiveTab={this.getUpdatedActiveTab}
              isActive={activeTab.tabId === tab.tabId}
            />
          ))}
        </ul>
        <ul className="apps-cont">
